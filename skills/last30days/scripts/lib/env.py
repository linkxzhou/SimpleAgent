...
def get_config() -> Dict[str, Any]:
    """Load configuration from environment variables and ~/.config/last30days/.env.

    Priority (high to low):
      1. OPENAI_* system default env vars (OPENAI_BASE_URL, OPENAI_API_KEY, OPENAI_MODEL)
      2. Explicit env vars (OPENAI_API_KEY, XAI_API_KEY, etc.)
      3. Config file (~/.config/last30days/.env)
    """
    # Load from config file first
    file_env = load_env_file(CONFIG_FILE)

    # --- System default OPENAI_* env vars ---
    t_openai_base_url = os.environ.get('OPENAI_BASE_URL', '').strip() or None
    t_openai_api_key = os.environ.get('OPENAI_API_KEY', '').strip() or None
    t_openai_model = os.environ.get('OPENAI_MODEL', '').strip() or None

    # Resolve OPENAI_API_KEY: OPENAI_API_KEY > OPENAI_API_KEY env > file
    openai_api_key = (
        t_openai_api_key
    ...
    # Resolve OPENAI_BASE_URL: OPENAI_BASE_URL > OPENAI_BASE_URL env > file > default
    openai_base_url = (
        t_openai_base_url
        or os.environ.get('OPENAI_BASE_URL')
        or file_env.get('OPENAI_BASE_URL')
        or None  # None means use provider default
    )

    # Resolve OPENAI_MODEL: OPENAI_MODEL > OPENAI_MODEL_PIN env > file
    # When OPENAI_MODEL is set, force policy to 'pinned'
    openai_model_pin = (
        t_openai_model
        or os.environ.get('OPENAI_MODEL_PIN')
        or file_env.get('OPENAI_MODEL_PIN')
    )
    openai_model_policy = 'pinned' if openai_model_pin else (
        os.environ.get('OPENAI_MODEL_POLICY')
    ...
    config = {
        ...
        'OPENAI_MODEL_POLICY': openai_model_policy,
        'OPENAI_MODEL_PIN': openai_model_pin,
