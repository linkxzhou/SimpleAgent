...
### System Default Environment Variables (Recommended)

If `OPENAI_*` system environment variables are already configured, the skill will **automatically use them** without any additional setup:

| Environment Variable | Description | Example |
|---------------------|-------------|---------|
| `OPENAI_BASE_URL` | OpenAI-compatible API base URL | `https://your-proxy.example.com/v1` |
| `OPENAI_API_KEY` | API key for the OpenAI-compatible service | `sk-xxx` |
| `OPENAI_MODEL` | Model name to use (skips auto-selection) | `gpt-4o` |

**Priority** (high → low): `OPENAI_*` env vars > `OPENAI_*` env vars > `~/.config/last30days/.env` file.

When `OPENAI_MODEL` is set, the skill will directly use the specified model without querying the models list API.

### First-Time Setup (Optional, for custom keys)
